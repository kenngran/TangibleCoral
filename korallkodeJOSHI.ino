...
// LED OUTPUT THROTTLE
// =========================================================
const uint16_t SHOW_INTERVAL_MS = 25; // ~40 FPS max output
...
uint16_t oldPhase = breath.phase;
breath.phase += (uint16_t)inc;
uint16_t newPhase = breath.phase;

bool wrapped = (newPhase < oldPhase);
if (wrapped) {
float oldCycleBright = breath.cycleMaxBright;
float oldCycleBpm = breath.cycleBpm;

breath.cycleMaxBright += (breath.pendingMaxBright - breath.cycleMaxBright) * breath.brightEasePerCycle;
breath.cycleBpm += (breath.pendingBpm - breath.cycleBpm) * breath.bpmEasePerCycle;
...
Serial.print("CYCLE WRAP -> cycleBright: ");
Serial.print(oldCycleBright, 2);
Serial.print(" -> ");
Serial.print(breath.cycleMaxBright, 2);
Serial.print(" | cycleBPM: ");
Serial.print(oldCycleBpm, 2);
Serial.print(" -> ");
Serial.println(breath.cycleBpm, 2);
}
...
const uint8_t MIN_BRIGHT_CHANGE = 1;
bool timeToShow = (nowMs - breath.lastShowMs) >= SHOW_INTERVAL_MS;
bool changedEnough = (abs((int)outBright - (int)breath.lastOut) >= MIN_BRIGHT_CHANGE);

if (timeToShow && changedEnough && !ultrasonicBusy()) {
fill_solid(leds, NUM_LEDS, c);
FastLED.show();

breath.lastShowMs = nowMs;
breath.lastOut = outBright;

Serial.print("SHOW | outBright=");
Serial.print(outBright);
Serial.print(" | cycleMaxBright=");
Serial.print(breath.cycleMaxBright, 2);
Serial.print(" | cycleBPM=");
Serial.print(breath.cycleBpm, 2);
Serial.print(" | pendingMaxBright=");
Serial.print(breath.pendingMaxBright, 2);
Serial.print(" | pendingBPM=");
Serial.println(breath.pendingBpm, 2);
}
}
